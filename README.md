...
- `moe-ssd-sim/v5` JSON export/import, deterministic Run ID, baseline diff, result·Advisor insight·completed parameter sweep replay 검증. V5 Run ID는 envelope schema, engine contracts, scheduler schema와 batch window에 결속됩니다. V4는 동일 build provenance일 때만 replay할 수 있으며 V3 이하는 호환하지 않습니다.
