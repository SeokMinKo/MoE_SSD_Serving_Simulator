...
const SWEEP_UNITS = Object.freeze({
  prompt: '토큰', output: '토큰', context: '토큰', conc: '시퀀스', seed: '정수', lat: 'µs', layers: '레이어', esize: 'MB/Expert', resident: 'GB', kvKB: 'KB/토큰', dcache: 'GB', minDCache: 'GB', vcache: 'GB', pinned: 'GB', page: 'GB', corr: '비율(0–1)', attn: 'ms/토큰', ems: 'ms/Expert', par: 'Expert', prefillSpeedup: '×', recall: '비율(0–1)', precision: '비율(0–1)', budget: 'MB/레이어', hidden: '채널', projections: '투영', chunks: '청크', bits: 'bit/가중치', packing: '×', commonGB: 'GB', freq: '토큰', overlap: '비율(0–1)', initSel: 'ms', periodicSel: 'ms', patchBase: 'ms', patchBW: 'GB/s', ffn: 'ms/토큰', runtime: 'ms/토큰', prefillTPS: '토큰/s', 'mem.backgroundGB': 'GB', 'mem.osReservedGB': 'GB', 'mem.minHeadroomGB': 'GB', 'mem.compressionRatio': '×', 'mem.compressionBW': 'GB/s', 'mem.swapCapacityGB': 'GB', 'mem.swapWriteRatio': '비율', 'mem.kvTouchFraction': '비율(0–1)'
});
...
const SWEEP_COMMON = Object.freeze([
  sweepDescriptor('prompt', 'Workload', SIMULATION_LIMITS.prompt.min, SIMULATION_LIMITS.prompt.max, { integer: true }), sweepDescriptor('output', 'Workload', SIMULATION_LIMITS.output.min, SIMULATION_LIMITS.output.max, { integer: true }), sweepDescriptor('context', 'Workload', 1, 10_000_000, { integer: true }), sweepDescriptor('conc', 'Workload', 1, 64, { integer: true }), sweepDescriptor('seed', 'Workload', 0, Number.MAX_SAFE_INTEGER, { integer: true }),
  ...
  sweepDescriptor('mem.compressionEnabled', 'Memory', 0, 0, { type: 'boolean', values: [false, true] }), sweepDescriptor('mem.compressionRatio', 'Memory', 1, 100), sweepDescriptor('mem.compressionBW', 'Memory', 0.001, 100_000), sweepDescriptor('mem.swapEnabled', 'Memory', 0, 0, { type: 'boolean', values: [false, true] }), sweepDescriptor('mem.swapCapacityGB', 'Memory', 0, 16_384), sweepDescriptor('mem.swapWriteRatio', 'Memory', SIMULATION_LIMITS.swapWriteRatio.min, SIMULATION_LIMITS.swapWriteRatio.max), sweepDescriptor('mem.kvTouchFraction', 'Memory', 0, 1)
]);
